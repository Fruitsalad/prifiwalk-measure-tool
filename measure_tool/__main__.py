...
def parse_args():
    ...
    parser.add_argument('--no-filestats',
                        help='Disable saving filetype statistics.',
                        dest='filestats',
                        action='store_false',
                        default=True)
...
def generate_histograms(wildfrag):
    ...
    for volume, _, _, _, _, _ in get_each_volume(wildfrag):
        derive_volume_statistics(volume, main_dir, size_cats)

    # Turn the data into histograms...
    for category, data in size_cats.items():
        generate_histogram_files(data, f"{main_dir}/{category}")
        for subcat, subdata in data.fullness_cats.items():
            generate_histogram_files(subdata, f"{main_dir}/{category}/{subcat}")

    # Create some extra histograms to show the distribution in size and fullness
    size_hist = Bins([1, 2, 3, 4, 5, 6, 7])

    for category, data in size_cats.items():
        fullness_hist = Bins([1, 2, 3, 4])


def derive_volume_statistics(volume, main_dir, size_cats):
    """
    ...
    if volume.used is not None:
        ...
        fullness = sum_file_sizes / volume.size
...
def generate_histogram_files(data, dir):
    """
